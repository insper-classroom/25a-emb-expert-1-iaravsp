...
static void control_led_protoboard(MQTT_CLIENT_DATA_T *state, bool on)
{
    // Publish state on /state topic and on/off led board
    const char *message = on ? "On" : "Off";
    if (on)
    ...
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/led_protoboard/state"), message, strlen(message), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}
...
static void publish_ldr(MQTT_CLIENT_DATA_T *state)
{
    ...
    static float old_ldr_value;
    const char *ldr_key = full_topic(state, "/ldr");
    float ldr_value = read_ldr();
    if (fabsf(ldr_value - old_ldr_value) > 0.01f)
    {
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T *state, bool sub)
{
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/led_protoboard"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    else if (strcmp(basic_topic, "/led_protoboard") == 0)
    {
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0)
            control_led_protoboard(state, true);
        else if (lwip_stricmp((const char *)state->data, "Off") == 0 || strcmp((const char *)state->data, "0") == 0)
            control_led_protoboard(state, false);
    }
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    ...
    if (status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        // Publish luminosidade every 2 sec if it's changed
